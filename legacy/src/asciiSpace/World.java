...
public class World {
	...
	public World(Ship pl)
	{
		...
		if(c == null)
			c = new Chunk((int)pl.getX()/PARTSIZE,(int)pl.getY()/PARTSIZE,(int)pl.getZ()/PARTSIZE);
	}
	...
	public void draw(boolean g0dm0de)
	{
		//int direction 1 = x 2 = y 3 = z
		if(prevX != (int)pl.getX()/PARTSIZE)
		{
			System.out.println("update X");
			c.update(1 * (((int)prevX-(pl.getX()/PARTSIZE) > 0)?1:-1), pl );
			prevX = (int)pl.getX()/PARTSIZE;
		}
		if(prevY != (int)pl.getY()/PARTSIZE && !g0dm0de)
		{
			System.out.println("update Y");
			c.update(2 * ((prevY-((int)pl.getY()/PARTSIZE) > 0)?1:-1), pl );
			prevY = (int)pl.getY()/PARTSIZE;
		}
		if(prevZ != (int)pl.getZ()/PARTSIZE)
		{
			System.out.println("update Z");
			c.update(3 * ((prevZ-((int)pl.getZ()/PARTSIZE) > 0)?1:-1), pl );
			prevZ = (int)pl.getZ()/PARTSIZE;
		}
		c.draw(pl);
	}
